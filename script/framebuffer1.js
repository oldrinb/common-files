    ...
    if (gl instanceof WebGLRenderingContext) {
      if (draw_ext) {
        ...
        for (let i = 0; i < nBuffers; i++) {
          ...
          gl.bindTexture(gl.TEXTURE_2D, this.textures[i].getWebGLTexture());
        ...
        gl.bindTexture(gl.TEXTURE_2D, this.textures[0].getWebGLTexture());
        gl.copyTexImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 0, 0, width, height, 0);
        gl.bindTexture(gl.TEXTURE_2D, null);
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
      }
      ...
      if (draw_ext) {
        for (let i = 0; i < textures.length; i++)
          gl.framebufferTexture2D(gl.FRAMEBUFFER,
              ...
              textures[i].getWebGLTexture(), 0);
      }
      else gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0,
               gl.TEXTURE_2D, textures[0].getWebGLTexture(), 0);
    }
    else for (let i = 0; i < textures.length; i++) {
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + i,
          gl.TEXTURE_2D, textures[i].getWebGLTexture(), 0);
    }
